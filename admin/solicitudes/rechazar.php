<?php
/**
 * Rechazar Solicitud de Préstamo (Administrador)
 */
...
if (!$requestId || empty($notes)) {
    AuthMiddleware::setFlash('error', 'Debes proporcionar un motivo para el rechazo');
    header('Location: index.php');
    exit();
}
...
$result = $requestModel->reject($requestId, $adminId, $notes);

if ($result['success']) {
    AuthMiddleware::setFlash('success', 'Solicitud rechazada');
} else {
